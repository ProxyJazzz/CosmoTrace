...
type FilterType = 'ALL' | 'UNMAPPED' | 'MAPPED' | 'X' | 'Y';

export const SensorAssignmentPanel = () => {
  const { calibrationMap, setCalibrationMap } = useAppStore();
  const { selectedSensorId, setSelectedSensorId, faultPreviewMode, setFaultPreviewMode, setHasUnsavedChanges } = useCalibrationStore();
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState<FilterType>('ALL');

  const channels = useMemo(() => {
    return Object.values(calibrationMap).sort((a, b) => {
      const aType = a.id[0];
      const bType = b.id[0];
  ...
  const filtered = channels.filter(c => {
    ...
    if (filter === 'X') return c.id.startsWith('X');
    if (filter === 'Y') return c.id.startsWith('Y');
    return true;
  });

  const handleUnmap = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (window.confirm(`Are you sure you want to unmap ${id}?`)) {
      ...
      <div className={styles.panelHeader}>
        ...
        <button 
          ...
          style={{
            ...
            padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', display: 'flex', gap: '4px', alignItems: 'center', fontSize: '0.75rem'
          }}
        >
          <Eye size={14} />
      ...
      <div style={{ padding: '1rem 1rem 0', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        <div style={{ position: 'relative' }}>
          ...
          <input 
            ...
            style={{ width: '100%', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border-color)', color: 'white', padding: '6px 8px 6px 28px', borderRadius: '4px' }}
          />
        </div>
        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
          {(['ALL', 'UNMAPPED', 'MAPPED', 'X', 'Y'] as FilterType[]).map(f => (
            <button 
              key={f}
              onClick={() => setFilter(f)}
              style={{
                ...
                padding: '2px 6px', borderRadius: '4px', fontSize: '0.65rem', cursor: 'pointer'
              }}
            >
              {f}
            </button>
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontWeight: 600, color: isSelected ? 'var(--text-main)' : 'var(--text-muted)' }}>{c.id}</span>
              </div>
